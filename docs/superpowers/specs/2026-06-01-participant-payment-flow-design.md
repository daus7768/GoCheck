...
- Slide-to-confirm payment submission
- Async message thread between participant and organizer
- Realtime updates to the participant page when organizer acts
- Organizer-side approve/reject UI in the existing invoice screen
...
- Payment gateway integration — participants still pay externally (DuitNow QR, bank transfer, etc.)
...
2. **Status card** — full-width `GlowingCard`, color and content driven by `paymentStatus`:
   ...
   - `pending` → amber accent, spinner + "Waiting for {organizer} to confirm"
   - `confirmed` → green accent, large check icon + "Paid ✓ on {confirmedAt}"
...
4. **Amount block** — `CountUp` formatted as `{currencySymbol}{amount}`, bill title under it, due date as small grey text
5. **Payment instructions card** — payment method label + details, long-press to copy. If method is `duitnow` and details include a QR data URL, render the QR image
6. **Proof upload (collapsible)** — `<input type="file">` on web, `expo-image-picker` on mobile. Thumbnail preview after upload, ✕ to remove. Subtext "Optional — helps {organizer} confirm faster"
7. **Slide-to-confirm bar** — visible only when `paymentStatus` ∈ `{ 'unpaid', 'rejected' }`. Implemented with `react-native-reanimated` PanGestureHandler. On commit: optional proof upload → `submit_payment` RPC → status flips to `pending` instantly (optimistic) → Realtime confirms
8. **Message thread** — scroll of messages (organizer left, participant right), composer at bottom (`<TextInput>` + send icon). Loads from `messages[]` in initial view, appends via Realtime
9. **Footer** — "Secure payment by GoCheck" + privacy line

**Components reused:** `GlowingCard`, `SheenButton`, `CountUp`, `FadeInUp`, `colors`, `typography`, `spacing`, `radius`, `shadow`.

**New components:**
- `src/components/payment/StatusCard.tsx` — status-driven card
- `src/components/payment/SlideToConfirm.tsx` — pan-gesture swipe bar
- `src/components/payment/MessageThread.tsx` — thread + composer
- `src/components/payment/ProofUpload.tsx` — proof picker (platform-branched)
...
- Their note (if any)
- Message thread preview + reply composer (reuses `MessageThread`)
